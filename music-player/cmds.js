...
module.exports = (MFBGB) => {
  ...
  let _registerDestructor = (guild, cnc) => {
  ...
  MFBGB.MusicPlayer.cmds.playYouTube = async ({ // Argument destructuring is easy to call a function when it's dequeued, though it's uneasy to read source codes
    ...
    let data = MFBGB.MusicPlayer.data[guild.id];
    if(data.disp !== null) await MFBGB.MusicPlayer.cmds.stop({ // When another music is already playing
      guild: guild,
      reason: "Starting another YouTube movie"
    });
    
    cnl.join().then(async cnc => {
      let ytStream = ytdl('https://www.youtube.com/watch?v=' + movieID, {filter: 'audioonly'});
  ...
  MFBGB.MusicPlayer.cmds.enqueue = (cmdName, cmdArgs) => {
    MFBGB.MusicPlayer.data[args.guild.id].queue.push({cmdName: cmdName, cmdArgs: cmdArgs});
  };
  ...
  MFBGB.MusicPlayer.cmds.stop = async ({
    ...
    if(fadeTime !== 0) await MFBGB.MusicPlayer.cmds.fadeVol({
      guild: guild,
      destVol: 0,
      fadeTime: fadeTime,
      dry: true
