...
module.exports = (MFBGB, message) => {
  if(!MFBGB.Doppelstimme.ready) return; // when 'ready' handler is still ongoing, ignore messages to me.
  if(message.author.id === MFBGB.Doppelstimme.user.id) return; // ignore messages I've sent
...
\`================================\``
    ...
    const level = MFBGB.Doppelstimme.permlevel(message);
    ...
    if(message.content.indexOf(MFBGB.config.prefix['Doppelstimme']) === 0){ // XPFaucet-Botのコマンド
      // Here we separate our "command" name, and our "arguments" for the command.
      // e.g. if we have the message "+say Is this the real life?" , we'll get the following:
      // command = say
      // args = ["Is", "this", "the", "real", "life?"]
      const args = message.content.slice(MFBGB.config.prefix['Doppelstimme'].length).trim().split(/ +/g);
      const command = args.shift().toLowerCase();

      // Check whether the command, or alias, exist in the collections defined
      const cmd = MFBGB.Doppelstimme.commands.get(command) || MFBGB.Doppelstimme.commands.get(MFBGB.Doppelstimme.aliases.get(command));
      ...
      if(level < MFBGB.Doppelstimme.levelCache[cmd.conf.permLevel]) {
        return message.channel.send(`:no_entry_sign: このコマンドを実行するのに必要な権限がありません。
あなたの権限レベル: ${level} (${levelName})
要求されている権限レベル: ${MFBGB.Doppelstimme.levelCache[cmd.conf.permLevel]} (${cmd.conf.permLevel})`);
      }
      ...
      MFBGB.Logger.cmd(`|Doppelstimme| ${message.author.username}(${message.author.id}) as ${levelName} ran '${cmd.help.name} ${args.join(" ")}'`);
