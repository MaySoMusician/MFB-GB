exports.run = async (client, message, args) => {
  ...
  const getWalletStatus = tickerCode => {
    return wr.get(client, 'https://www.coinexchange.io/api/v1/getcurrency', {ticker_code: tickerCode}, true)
      .then(res => {
  ...
  const getMarketID = (assetName, baseCurrency) => {
    ...
    if (marketInfosCache === null) {
      return wr.get(client, 'https://www.coinexchange.io/api/v1/getmarkets', {}, true)
        .then(res => {
          client.Logger.log('|BS-Discord| [via CoinExchange] Successfully received all market data');
      ...
      client.Logger.log('|BS-Discord| Cached market data will be used');
      const data = marketInfosCache.find(funcFind);
  ...
  const getMarketSummary = marketId => {
    return wr.get(client, 'https://www.coinexchange.io/api/v1/getmarketsummary', {market_id: marketId}, true)
      .then(res => {
  ...
  const getCurrencyExchangeRate = (currency, baseCurrencyTicker) => {
    return wr.get(client, 'https://api.coingecko.com/api/v3/simple/price',
  ...
  client.Logger.log(`|BS-Discord| [via CoinExchange] Wallet status: ${walletStatus}`);
  ...
  client.Logger.log(`|BS-Discord| [via CoinGecko] BTC/USD = ${rateBtcUsd} (eq. to sat/USD = ${rateSatoshiUsd})`);

  // Get XPC/DOGE market id
  infoXpcDoge.marketId = await getMarketID('eXPerience Chain', 'Dogecoin');
  if (infoXpcDoge.marketId !== null) {
    client.Logger.log(`|BS-Discord| [via CoinExchange] XPC/DOGE MarketID: ${infoXpcDoge.marketId}`);
    ...
    client.Logger.log(`|BS-Discord| [via CoinExchange] XPC/DOGE Bid: ${infoXpcDoge.bid}, Ask: ${infoXpcDoge.ask}, Vol: ${infoXpcDoge.vol} (eq. to ${infoXpcDoge.btcVol} BTC)`);
    ...
    client.Logger.log(`|BS-Discord| [via CoinGecko] DOGE/BTC = ${rateDogeBtc} (eq. to DOGE/sat = ${rateDogeSatoshi})`);
    ...
    client.Logger.log(`|BS-Discord| Virtual XPC/satoshi Bid: ${infoXpcDogeSatoshi.bid}, Ask: ${infoXpcDogeSatoshi.ask}`);
    ...
    client.Logger.log(`|BS-Discord| Virtual XPC/USD (DOGE) Bid: ${infoXpcDogeSatoshi.usdBid}, Ask: ${infoXpcDogeSatoshi.usdAsk}`);
...
取引高: ${infoXpcDoge.vol} DOGE (${infoXpcDoge.btcVol} BTC)`;
    ...
    client.Logger.warn(`|BS-Discord| [via CoinExchange] XPC/DOGE MarketID: Not found`);
    strXpcDoge = `**[DOGE建て]**
(上場廃止)`;
  }
  ...
  client.Logger.log(`|BS-Discord| [via CoinExchange] XPC/BTC MarketID: ${infoXpcBtc.marketId}`);
  ...
  client.Logger.log(`|BS-Discord| [via CoinExchange] XPC/BTC Bid: ${infoXpcBtc.bid}, Ask: ${infoXpcBtc.ask}, Vol: ${infoXpcBtc.vol} BTC`);
  ...
  client.Logger.log(`|BS-Discord| Virtual XPC/USD (BTC) Bid: ${infoXpcBtc.usdBid}, Ask: ${infoXpcBtc.usdAsk}`);
